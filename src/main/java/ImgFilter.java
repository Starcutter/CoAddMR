...
public class ImgFilter {
	...
	public static class picInfo {
		...
		public String toString() {
			return "/home/zhaohadoop/summer/project/301/" + run + "/" + camcol + "/frame-g-" + 
					String.format("%06d", run) + "-" + camcol + "-" + String.format("%04d", field) + ".fits";
		}
		
		picInfo(String init[]) {
			this.run = Integer.parseInt(init[0]); this.field = Integer.parseInt(init[1]); this.camcol = Integer.parseInt(init[2]);
			for(int i = 0; i < 4; ++i) {
				this.coordinate[i] = Double.parseDouble(init[i + 4]);
			}
		}
	}
