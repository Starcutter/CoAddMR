...
由于中途失去了集群，我们最终在单机上配置伪分布式 Hadoop 系统来运行项目。软硬件配置如下：
...
系统功能是正确的；效率上，数据预处理与预过滤已经大大减少了 Mapper 数量，成倍提升了效率，但目前效率瓶颈仍主要有两点：

第一，预过滤力度不足拖长了 map 过程的时间。前面提到，由于只考虑了赤纬而没有考虑赤经，导致预过滤留下了很多假阳性的数据。这是数据组织形式（按 camcol, band 存储 Sequence File）所决定的。如果要解决，可以在预处理时让 Sequence File 内部按照 field 序号有序排列（field 对应赤经范围），这样就可以在 InputFormat 内执行二分查找，减少 map 处理的数据量。

第二，Reducer 缓慢。系统原理决定了只能由一个 Reducer 处理一个查询，无论 Mapper 如何并行，单个查询的 Reducer 也无法由并行来解决效率问题。

另外，也是由于单个 Reducer 处理单个查询的原理限制，如果查询范围过大，那么 Reducer 就会收到巨量的待叠加图像数据，会导致 Reducer 资源占用过多而崩溃。这个问题目前还不知道如何解决。

## 总结与展望

本系统正确地实现了预想的功能：对大量天文图像，查询指定赤经-赤纬范围与波段，输出叠加后的图像，并支持并行查询。系统能够在保持图像质量的前提下，拼接出指定范围的图像，并改善星体的可视度。系统通过预处理与预过滤的方式，提高了查询的效率，但仍存在着预过滤力度不足、单个 Reducer 缓慢导致的性能瓶颈，以及查询范围不能做到很大的问题。

未来的工作还可以有：

- 前述的进一步预处理，通过二分查找缩小 map 处理的数据量。
- 提高合成图像质量：忽略或降权质量差的原始图片
- 图像上色：支持查询多个波段，并根据各个波段的占比进行假彩色上色。
